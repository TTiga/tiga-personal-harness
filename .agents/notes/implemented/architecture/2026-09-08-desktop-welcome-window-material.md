...
The Tiga desktop surgery later removed the welcome window, its renderer, and the account backend that served it; Desktop startup now enters the main window directly and the in-workspace DeepSeek onboarding dialog is the only first-configuration path. The reads this note relied on — shared `locale.preference` and API-key presence over the Web settings and credential RPC — survive in `apps/desktop/src/workspace-bootstrap.ts`. Reintroducing a pre-workspace credential surface would need to rebuild a startup owner and window from scratch; this note keeps the material and ownership rationale for that case.
